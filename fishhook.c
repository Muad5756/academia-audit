...
static void rebind_symbols_for_image(struct rebindings_entry *rebindings,
    ...
    for (uint i = 0; i < header->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
        ...
        if (cur_seg_cmd->cmd == LC_SEGMENT_ARCH_DEPENDENT) {
            if (!strcmp(cur_seg_cmd->segname, SEG_LINKEDIT)) {
    ...
    for (uint i = 0; i < header->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
        ...
        if (!strcmp(cur_seg_cmd->segname, SEG_DATA) ||
            ...
            section_t *section = (section_t *)((char *)cur_seg_cmd + sizeof(segment_command_t));
            for (uint j = 0; j < cur_seg_cmd->nsects; j++, section++) {
                if ((section->flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS ||
                    (section->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS) {
                    perform_rebinding_with_section(rebindings, section,
